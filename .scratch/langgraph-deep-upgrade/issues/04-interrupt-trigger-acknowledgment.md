...
RiskGuardianAgent 评估为 HIGH 时，通过 LangGraph `interrupt()` 中断图执行（停在 CounselorAgent 生成回复之前）。`run()` 的返回区分"已完成"（含 response_messages）和"待审核"（被 interrupt）两种状态。ChatService 收到"待审核"时发送含危机支持资源的确认消息并关闭 SSE 流，同时创建待审队列实体（关联 session、PsychologicalReport、风险摘要、状态 pending）。CHAT 路径不进入 RiskGuardianAgent，不受影响。Custom runtime 无 interrupt，HIGH 保留直接回复。
...
- [ ] Custom runtime（LangGraph 不可用时）HIGH 仍直接回复，不 interrupt
